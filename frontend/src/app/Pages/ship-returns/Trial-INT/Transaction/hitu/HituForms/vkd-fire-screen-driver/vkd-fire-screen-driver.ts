...
@Component({
  selector: 'app-vkd-fire-screen-driver',
  imports: [
    CommonModule,
    ReactiveFormsModule,
    FormCardComponent,
  ...
  standalone: true,
  templateUrl: './vkd-fire-screen-driver.html',
  styleUrl: './vkd-fire-screen-driver.css',
})
export class VkdFireScreenDriver {
  editMode = false;
  rowId!: string | null;
  ...
  setupConditionalLogic() {
    ...
    this.form.get('lastDateOfOilChange')?.valueChanges.subscribe((value) => {
  ...
  private async loadTrialPrefillFromQuery(): Promise<void> {
    ...
    try {
      ...
      console.error('Trial prefill failed (VKD Fire Screen Driver)', e);
    }
  }
  ...
  async setActiveTab(tab: any): Promise<void> {
    ...
    try {
      ...
      console.error(
        'Failed to load VKD Fire Screen Driver data for selected equipment',
        error,
      );
  ...
  private resetFormData(): void {
    ...
    this.showOperationalTrialsInput = false;
    this.showGreasePointsInput = false;
    this.showOilGearBoxOthersInput = false;

    this.form.patchValue({ ship }, { emitEvent: false });
  }

  /** Poore form ko equipment-specific payload se generic tarike se hydrate karta hai,
   *  aur conditional-lock wale remark fields ka disabled state bhi manually restore
   *  karta hai (kyunki emitEvent:false hone ki wajah se valueChanges listeners
   *  trigger nahi hote patch ke time). */
  fillData(payload: any): void {
    ...
    // ---- Conditional visibility flags + disabled remark controls restore ----
    ...
    // Ser 8: Oil being used in Gear Box
    if (payload.oil_gear_box_type === 'Others') {
      this.showOilGearBoxOthersInput = true;
    }
    if (
      payload.oil_gear_box_value === 'Yes' ||
      payload.oil_gear_box_value === 'No'
    ) {
      this.form.get('oil_gear_box_remark')?.disable({ emitEvent: false });
    }

    // Ser 9: Oil Level
    if (payload.oil_level) {
      this.form.get('oil_level_remark')?.disable({ emitEvent: false });
    }

    // Ser 11: Change of Oil
    if (payload.lastDateOfOilChange) {
      this.form.get('lastOilChangeDate_marks')?.disable({ emitEvent: false });
    }
