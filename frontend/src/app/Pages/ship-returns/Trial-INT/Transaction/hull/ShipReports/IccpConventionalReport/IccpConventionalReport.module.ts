...
@Component({
  selector: 'app-quarterly-hull-iccp-conventional-report',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './IccpConventionalReport.module.html',
  styleUrls: ['../report.module.css'],
})
export class QuarterlyHullIccpConventionalReportComponent implements OnInit {
  rowId!: string;
  ...
  getReportData(id: string): void {
    this.apiService
      .get(
        `${Apiendpoints.QUARTERLY_HULL_POTENTIAL_DATA_OF_SHIPS_FITTED_WITH_CONVENTIIONAL_ICCP_SYSTEM}${id}/`,
      )
      .subscribe({
        next: (res: any) => {
          if (res?.data) {
            ...
            this.presetPotentialIccpAcu = data?.preset_potential_iccp_acu || '-';
            this.oemIccpSystem = data?.oem_iccp_system || '-';
            this.totalNoOfAnodes = data?.total_no_of_anodes || '-';
            this.noOfREs = data?.no_of_res || '-';
            ...
            /* Table 1: iccpMeasurements */
            this.iccpMeasurements = (
              data?.quarterly_hull_conventional_iccp_log ||
              data?.quarterly_hull_conventional_iccp_log_readings ||
              []
            ...
            /* Table 2: sacrificalAnodes */
            this.sacrificalAnodes = (
              data?.quarterly_hull_conventional_iccp_anode ||
              data?.quarterly_hull_conventional_iccp_anode_anodes ||
              []
            ).map((item: any, index: number) => ({
              id: item?.id || null,
              s_no: index + 1,
            ...
            /* Table 3: acuOffReadings */
            this.acuOffReadings = (
              data?.quarterly_hull_conventional_iccp_acu_off ||
              data?.quarterly_hull_conventional_iccp_acu_off_readings ||
              []
            ).map((item: any, index: number) => ({
              id: item?.id || null,
              s_no: index + 1,
            ...
            /* Table 4: acuOnReadings */
            this.acuOnReadings = (
              data?.quarterly_hull_conventional_iccp_acu_on ||
              data?.quarterly_hull_conventional_iccp_acu_on_readings ||
              []
            ).map((item: any, index: number) => ({
              id: item?.id || null,
              s_no: index + 1,
        ...
        error: (err: any) => {
          console.error('Error fetching ICCP Conventional report:', err);
        },
      });
  }
