...
// import { FormInputTableWithHeaders } from '../../../../../Components/form-input-table-with-headers/form-input-table-with-headers.component';
...
function lessThanValidator(max: number) {
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    if (num >= max) {
      return { lessThan: { max, actual: num } };
    }
    if (num < 0) {
...
@Component({
  selector: 'app-part-a-induction-survey-report',
  standalone: true,
  host: {
  ...
  templateUrl: './part-a-induction-survey-report.html',
  styleUrl: './part-a-induction-survey-report.css',
})
export class PartAInductionSurveyReport implements OnInit, OnDestroy {
  ...
  /** Shared equipment list for BOTH axes (Monitor + Active) */
  matrixEquipmentOptions: SelectOption[] = [
    { label: 'Radar', value: 'Radar' },
    { label: 'HF Radio', value: 'HF Radio' },
    { label: 'VHF Radio', value: 'VHF Radio' },
    { label: 'UHF Radio', value: 'UHF Radio' },
    { label: 'EW System', value: 'EW System' },
    { label: 'Sonar', value: 'Sonar' },
    { label: 'Navigation System', value: 'Navigation System' },
    { label: 'Satellite Communication', value: 'Satellite Communication' },
    { label: 'IFF System', value: 'IFF System' },
  ...
  /** Options for "Compartment" dropdown in SE Checks table */
  // compartmentOptions: SelectOption[] = [
  //   { label: "Cat 'A' Compartment 1", value: "Cat 'A' Compartment 1" },
  //   { label: "Cat 'A' Compartment 2", value: "Cat 'A' Compartment 2" },
  //   { label: "Cat 'A' Compartment 3", value: "Cat 'A' Compartment 3" },
  //   { label: 'Operations Room', value: 'Operations Room' },
  //   { label: 'Bridge', value: 'Bridge' },
  //   { label: 'Radio Room', value: 'Radio Room' },
  //   { label: 'Sonar Room', value: 'Sonar Room' },
  //   { label: 'Machinery Control Room', value: 'Machinery Control Room' },
  ...
  compartmentOptions: SelectOption[] = [];
  /** Options for "System" dropdown in VSWR Checks */
  vswrSystemOptions: SelectOption[] = [
    { label: 'HF Antenna', value: 'HF Antenna' },
    { label: 'VHF Antenna', value: 'VHF Antenna' },
    { label: 'UHF Antenna', value: 'UHF Antenna' },
    { label: 'Radar Antenna', value: 'Radar Antenna' },
    { label: 'IFF Antenna', value: 'IFF Antenna' },
    { label: 'EW Antenna', value: 'EW Antenna' },
    { label: 'Satellite Antenna', value: 'Satellite Antenna' },
    { label: 'Navigation Antenna', value: 'Navigation Antenna' },
    { label: 'Other', value: 'Other' },
  ];

  /** Options for "Equipment" dropdown in Visual Survey Observation */
  // equipmentOptions: SelectOption[] = [
  //   { label: 'Radar', value: 'Radar' },
  //   { label: 'HF Radio', value: 'HF Radio' },
  //   { label: 'VHF Radio', value: 'VHF Radio' },
  ...
  /** Options for "Equipment" dropdown in Grounding Discontinuity Checks */
  groundingEquipmentOptions: SelectOption[] = [
    { label: 'Switch Board', value: 'Switch Board' },
    ...
    { label: 'Radar', value: 'Radar' },
    { label: 'HF Radio', value: 'HF Radio' },
    { label: 'Antenna Mast', value: 'Antenna Mast' },
    { label: 'Other', value: 'Other' },
  ];

  /** Options for "Compt" dropdown in Grounding Discontinuity Checks */
  comptOptions: SelectOption[] = [
    { label: 'Bridge', value: 'Bridge' },
    { label: 'Engine Room', value: 'Engine Room' },
    { label: 'Machinery Space', value: 'Machinery Space' },
    { label: 'Radio Room', value: 'Radio Room' },
    { label: 'Ops Room', value: 'Ops Room' },
  ...
  //     console.log("check trial row", trialRow);
  //     if (!trialRow) return;
  //     if (trialRow.ship_id != null) {
  //       this.form.get('ship')?.setValue(String(trialRow.ship_id), { emitEvent: false });
  //     }
  //   } catch (e) {
  //     console.error('Trial prefill failed (NEC Part A Induction Survey Report)', e);
  //   }
  // }
  private async loadTrialPrefillFromQuery(): Promise<void> {
    const trialId = resolveTrialQueryParam(this.route, this.router);
    ...
    try {
      ...
      const jsonData =
        response?.json_data ?? // if data is coming wrraped
      ...
      console.log('Final payload sent to fillData:', equipmentPayload);
      ...
      console.error(
        'Trial prefill failed (NEC Part A Induction Survey Report)',
        e,
      );
    }
  }
  ...
  async setActiveTab(tab: any): Promise<void> {
    ...
    try {
      ...
      console.log('RAW response from getFormByEquipment:', response);
      const jsonData =
      ...
      console.error(
        'Failed to load NEC Part A data for selected equipment',
        error,
      );
  ...
  /** Fetches systems from master/systems/ and populates both EMI dropdown arrays */
  ...
  //   this.apiService
  //     .getDropdownData('master/equipments/', { labelKey: 'name', valueKey: 'id' })
  //     .subscribe((options) => {
  ...
  addSeCompartmentRow(): void {
    ...
    const newIndex = this.seCompartmentsArray.length - 1;
  ...
  removeInterferenceRow(index: number): void {
    if (index < 1) return; // only allow removing rows beyond the first 11
   ...
   * Mirrors the ak-190-health-audit `fillData()` pattern.
   */
