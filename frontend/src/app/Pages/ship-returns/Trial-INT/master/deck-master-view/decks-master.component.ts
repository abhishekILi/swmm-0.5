...
import { DecksMasterAddComponent } from '../deck-master-add/decks-master-add.component';

@Component({
  selector: 'app-decks-master',
  standalone: true,
  templateUrl: './decks-master.component.html',
  imports: [
    CommonModule,
    PaginateTableComponent,
    DecksMasterAddComponent,
    ReusableDeleteDialogComponent,
  ],
})
export class DecksMasterComponent implements OnInit {
  page = 1;
  totalPages = 0;
  totalCount = 0;
  ...
  columnDefs = [
    ...
    { field: 'classofship_name', headerName: 'Class Name', filter: 'agTextColumnFilter' },
    { field: 'deck_no', headerName: 'Deck', filter: 'agTextColumnFilter' },
    {
      headerName: 'Status',
      sortable: false,
      filter: false,
...
openDeleteDialog(row: any) {
  ...
  fetchUnitMaster(pageNum: number = 1 , size : number = 10): void {
    ...
    this.apiService
      .get(`${Apiendpoints.MASTER_DECKS}?page=${pageNum}&page_size=${size}`)
      .subscribe({
        next: (res: any) => {
