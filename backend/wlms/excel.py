...
class ExcelImportExportMixin:
    ...
    def import_excel(self, request):
        ...
        from wlms.tasks import import_excel_task

        file_path = save_uploaded_file(uploaded_file, "wlms-imports")
        ...
        if dispatch["queued"]:
            return accepted_task_response(
                request,
                dispatch["task"],
                "WLMS Excel import queued for background processing.",
            )
        return sync_task_response(
            dispatch["result"],
            "Background task service unavailable. WLMS Excel import completed synchronously.",
        )
